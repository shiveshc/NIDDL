...
class Hourglass_wres(nn.Module):
    def __init__(self, in_channels, out_channels=32):
        super().__init__()
        self.inconv = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding='same')
        self.relu = nn.ReLU()

        self.db1 = ConvBlock(out_channels, out_channels)
        ...
        self.sb1 = ConvBlock(out_channels, out_channels)
        self.sb2 = ConvBlock(out_channels, out_channels)
        self.sb3 = ConvBlock(out_channels, out_channels)

        self.ub1 = ConvBlockTranspose(out_channels, out_channels)
        self.cb1 = ConvBlock(out_channels, out_channels)
        self.ub2 = ConvBlockTranspose(out_channels, out_channels)
        self.cb2 = ConvBlock(out_channels, out_channels)
        self.ub3 = ConvBlockTranspose(out_channels, out_channels)
        self.cb3 = ConvBlock(out_channels, out_channels)
    ...
    def forward(self, x):
        conv0 = self.relu(self.inconv(x))

        conv1, conv2, conv3 = self.db1(conv0)
