...
class UNet_v2(nn.Module):
    def __init__(self, in_channels, out_channels=32):
        ...
        self.db5 = ConvBlock(8*out_channels, 16*out_channels)
        self.maxpool = nn.MaxPool2d(2, 2)
        
        self.ub1 = ConvBlockTranspose(16*out_channels, 16*out_channels)
        self.cb1 = ConvBlock((16+8)*out_channels, 8*out_channels)
        self.ub2 = ConvBlockTranspose(8*out_channels, 8*out_channels)
        self.cb2 = ConvBlock((8+4)*out_channels, 4*out_channels)
        self.ub3 = ConvBlockTranspose(4*out_channels, 4*out_channels)
        self.cb3 = ConvBlock((4+2)*out_channels, 2*out_channels)
        self.ub4 = ConvBlockTranspose(2*out_channels, 2*out_channels)
        self.cb4 = ConvBlock((2+1)*out_channels, out_channels)
