...
def conv_net(x, output_shape):
    ...
    # sideblock1
    conv10, conv11, conv12, weights, biases = residual_block(conv3, weights, biases, 32)

    # sideblock2
    conv13, conv14, conv15, weights, biases = residual_block(conv6, weights, biases, 32)

    # up1
    mp4 = maxpool2d(conv15, 2)
    conv9 = conv9 + mp4
    weights, biases, new_w, new_b = make_wts_and_bias(weights, biases, 32, 32, 'transpose')
    conv16 = conv2d_transpose(conv9, weights[new_w], biases[new_b], [tf.shape(conv3)[0], conv6.shape[1], conv6.shape[2], 32], strides=2)
    conv17, conv18, conv19, weights, biases = residual_block(conv16, weights, biases, 32)

    # up2
    mp5 = maxpool2d(conv12, 2)
    conv19 = conv19 + mp5
    weights, biases, new_w, new_b = make_wts_and_bias(weights, biases, 32, 32, 'transpose')
    conv20 = conv2d_transpose(conv19, weights[new_w], biases[new_b], [tf.shape(conv3)[0], conv3.shape[1], conv3.shape[2], 32], strides=2)
    conv21, conv22, conv23, weights, biases = residual_block(conv20, weights, biases, 32)

    weights, biases, new_w, new_b = make_wts_and_bias(weights, biases, 32, output_shape, 'normal')
    conv24 = conv2d(conv23, weights[new_w], biases[new_b])
    out = conv24
    return out