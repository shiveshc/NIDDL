...
def conv_net(x):
    ...
    conv22 = conv2d_transpose(conv12, weights[new_w], biases[new_b], [tf.shape(conv12)[0], 32, 32, 32], strides=2)
    ...
    conv25 = conv25 + mp5
    weights, biases, new_w, new_b = make_wts_and_bias(weights, biases, 32, 32, 'transpose')
    conv26 = conv2d_transpose(conv25, weights[new_w], biases[new_b], [tf.shape(conv12)[0], 64, 64, 32], strides=2)
    ...
    conv29 = conv29 + mp6
    weights, biases, new_w, new_b = make_wts_and_bias(weights, biases, 32, 32, 'transpose')
    conv30 = conv2d_transpose(conv29, weights[new_w], biases[new_b], [tf.shape(conv12)[0], 128, 128, 32], strides=2)
