...
def make_wts_and_bias(weights, biases, input_channels, output_channels, type):
    ...
    if type == 'normal':
        ...
        weights[new_w] = tf.get_variable('W' + str(curr_weights_num + 1), shape=(5, 5, input_channels, output_channels), initializer=tf.contrib.layers.xavier_initializer())
        ...
        weights[new_w] = tf.get_variable('W' + str(curr_weights_num + 1), shape=(5, 5, output_channels, input_channels), initializer=tf.contrib.layers.xavier_initializer())
...
def conv_net(x, output_shape):
    ...
    uconv1 = conv2d_transpose(conv15, weights[new_w], biases[new_b], [tf.shape(conv3)[0], 64, 64, 32], strides=2)
    uconv1 = tf.concat([uconv1, conv12], axis=3)
    conv16, conv17, conv18, weights, biases = conv_block(uconv1, weights, biases, 32)

    # up block 2
    weights, biases, new_w, new_b = make_wts_and_bias(weights, biases, 32, 32, 'transpose')
    uconv2 = conv2d_transpose(conv18, weights[new_w], biases[new_b], [tf.shape(conv3)[0], 128, 128, 32], strides=2)
    uconv2 = tf.concat([uconv2, conv9], axis=3)
    conv19, conv20, conv21, weights, biases = conv_block(uconv2, weights, biases, 32)

    # up block 3
    weights, biases, new_w, new_b = make_wts_and_bias(weights, biases, 32, 32, 'transpose')
    uconv3 = conv2d_transpose(conv21, weights[new_w], biases[new_b], [tf.shape(conv3)[0], 256, 256, 32], strides=2)
    uconv3 = tf.concat([uconv3, conv6], axis=3)
    conv22, conv23, conv24, weights, biases = conv_block(uconv3, weights, biases, 32)

    # up block 4
    weights, biases, new_w, new_b = make_wts_and_bias(weights, biases, 32, 32, 'transpose')
    uconv4 = conv2d_transpose(conv24, weights[new_w], biases[new_b], [tf.shape(conv3)[0], 512, 512, 32], strides=2)
