...
def conv_net(x):
    ...
    uconv1 = conv2d_transpose(conv15, weights[new_w], biases[new_b], [tf.shape(conv3)[0], 64, 64, 512], strides=2)
    ...
    uconv2 = conv2d_transpose(conv18, weights[new_w], biases[new_b], [tf.shape(conv3)[0], 128, 128, 256], strides=2)
    ...
    uconv3 = conv2d_transpose(conv21, weights[new_w], biases[new_b], [tf.shape(conv3)[0], 256, 256, 128], strides=2)
    ...
    uconv4 = conv2d_transpose(conv24, weights[new_w], biases[new_b], [tf.shape(conv3)[0], 512, 512, 64], strides=2)
