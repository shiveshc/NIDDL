...
def conv_net(x):
    ...
    # sideblock1
    conv13, conv14, conv15, weights, biases = residual_block(conv3, weights, biases, 64)

    # sideblock2
    conv16, conv17, conv18, weights, biases = residual_block(conv6, weights, biases, 128)

    # sideblock3
    conv19, conv20, conv21, weights, biases = residual_block(conv9, weights, biases, 256)

    # up1
    mp4 = maxpool2d(conv21, 2)
    conv12 = conv12 + mp4
    weights, biases, new_w, new_b = make_wts_and_bias(weights, biases, 256, 256, 'transpose')
    conv22 = conv2d_transpose(conv12, weights[new_w], biases[new_b], [tf.shape(conv12)[0], 128, 128, 256], strides=2)
    conv23, conv24, conv25, weights, biases = residual_block(conv22, weights, biases, 128)

    # up2
    mp5 = maxpool2d(conv18, 2)
    conv25 = conv25 + mp5
    weights, biases, new_w, new_b = make_wts_and_bias(weights, biases, 128, 128, 'transpose')
    conv26 = conv2d_transpose(conv25, weights[new_w], biases[new_b], [tf.shape(conv12)[0], 256, 256, 128], strides=2)
    conv27, conv28, conv29, weights, biases = residual_block(conv26, weights, biases, 64)

    # up3
    mp6 = maxpool2d(conv15, 2)
    conv29 = conv29 + mp6
    weights, biases, new_w, new_b = make_wts_and_bias(weights, biases, 64, 64, 'transpose')
    conv30 = conv2d_transpose(conv29, weights[new_w], biases[new_b], [tf.shape(conv12)[0], 512, 512, 64], strides=2)
